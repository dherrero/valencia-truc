  ...
  return (
    <div className="min-h-screen bg-emerald-950 text-white flex flex-col items-center px-4 py-12">
      ...
      <div className="fixed inset-0 opacity-5 pointer-events-none"
        style={{ backgroundImage: 'repeating-linear-gradient(0deg, #10b981 0, #10b981 1px, transparent 0, transparent 40px), repeating-linear-gradient(90deg, #10b981 0, #10b981 1px, transparent 0, transparent 40px)' }}
      />

      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 w-full max-w-3xl">
        {/* Header */}
        <div className="text-center mb-12">
          <h1 className="text-6xl font-black mb-2" style={{ textShadow: '0 0 40px rgba(52,211,153,0.4)' }}>
          ...
          <p className="text-emerald-400">
            ...
            ) : <span className="text-red-400">Connectant…</span>}
          </p>
        </div>

        {/* Room list */}
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-emerald-300">Partides actives</h2>
          <button
        ...
        <AnimatePresence mode="popLayout">
          {rooms.length === 0 ? (
            ...
            >
              No hi ha partides actives. Crea la primera!
            </motion.div>
          ) : (
                ...
                >
                  <div>
                    ...
                    <p className="text-emerald-400 text-sm mt-0.5">
                      ...
                      {' · '}
                      <span className={room.status === 'playing' ? 'text-yellow-400' : 'text-emerald-400'}>
                        {room.status === 'playing' ? 'En joc' : 'Esperant'}
                      </span>
                    </p>
                  </div>
                  <button
                  ...
                  >
                    {room.playerCount >= room.maxPlayers ? 'Plena' : 'Unir-se'}
                  </button>
                </motion.div>
              ))}
            </div>
      ...
      <AnimatePresence>
        {showModal && (
          ...
          >
            ...
            >
              <h3 className="text-2xl font-black mb-6">Nova Sala</h3>

              <label className="block text-emerald-300 text-sm font-semibold mb-1">Nom de la sala</label>
              <input
                type="text"
                value={roomName}
              ...
              <label className="block text-emerald-300 text-sm font-semibold mb-3">
                Bots rivals: <span className="text-white font-black text-lg">{botCount}</span>
              </label>
